...
| `--remote-control` | `true` | Enable Remote Control on Claude runs so a headless agent is viewable, and steerable, from the Claude mobile app and claude.ai/code. Claude receives `--settings {"remoteControlAtStartup":true}`, which starts the Remote Control bridge and is layered on top of your own settings, plus `--rc "glorp owner/repo#N"` so concurrent runs are told apart by issue instead of sharing a host name. Codex runs are unaffected and stay viewable in glorp's own dashboard only, so a watch that has Codex configured prints that once at startup rather than leaving the run's absence from the phone unexplained. `codex exec` takes no Remote Control argument, and Codex's separate `codex remote-control` daemon is not a way around that: it is the app-server hosting the threads it starts itself, so glorp has nothing to hand it an already-running `codex exec`, its lifecycle commands are Unix-only, and pairing a device to it is a manual step. Reaching Codex runs from a phone would mean driving Codex over the app-server protocol instead of `codex exec` altogether. Turn it off with `--remote-control=false`. |
