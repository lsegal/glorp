...
Naming no model runs the agent's managed default -- `gpt-5.6-terra` for `codex`, `sonnet` for `claude` -- rather than whatever its CLI reaches for on its own, which is usually its largest model. `glorp agents` prints each default, and a definition can set or drop its own with `"defaultModel"` in `--config`.
