...
| `--claude-binary PATH` | `claude` | Claude Code executable name or path. An alias for `--agent-binary claude=PATH`, kept for the scripts that already pass it; `--agent-binary` wins when both are given. |
| `--codex-binary PATH` | `codex` | Codex executable name or path. An alias for `--agent-binary codex=PATH`, kept for the scripts that already pass it; `--agent-binary` wins when both are given. |
