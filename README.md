...
| --- | --- | --- |
| `--agent AGENT[/MODEL][:LEVEL]` | `codex` | Agent to run, with an optional model and reasoning level, such as `claude`, `claude/opus`, or `codex/gpt-5.6:high`. glorp ships definitions for `codex` and `claude`, both accepting the levels `low`, `medium`, and `high`; `--config` adds or overrides agents without a rebuild, and an unrecognised name lists the ones that are defined. Repeatable; when given more than once, new issues are load balanced evenly across the listed agents, each using its own model and level. |
...
| `--config PATH` | `.glorp.config.json` | Agent definition file. Each agent glorp can dispatch to -- its executable, the arguments for a fresh run, a resume, and a one-shot vision read, its environment, how its session ID is established, and how its output is decoded -- is described by JSON rather than by code, so overriding one of the built-in agents or adding a CLI glorp has never heard of is a file rather than a rebuild. The file is a top-level object with an `agents` key holding either an array of definitions or an object keyed by agent name; a definition whose name matches a built-in overrides only the fields it states, and any other name registers a new agent that `--agent` then accepts. A missing file is not an error, and glorp never writes this one, so hand-edited definitions cannot be clobbered -- it is deliberately separate from the `--state` work-state file, and each of the two says so if it is handed the other's contents. A malformed file, an unknown field, or an invalid value fails at startup naming the file, the agent, and the field, since a definition that was silently dropped looks exactly like a typo in `--agent`. |
