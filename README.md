...
For repository targets, opening an issue yourself or assigning one to yourself marks it eligible for pickup; glorp's default filter dispatches open issues that the authenticated user is assigned to or opened. Ownership of a claimed issue is tracked entirely through the comment-based handoff protocol below rather than a label. Project items are moved through their configured status as work starts and finishes.
...
| `--filter QUERY` | `is:issue state:open assignee:@me` | GitHub issue-search filter. Repeat the option to combine terms. The default applies to repository targets and matches open issues assigned to you or opened by you (`assignee:@me` and `author:@me` are searched separately and unioned, since GitHub issue search has no `OR`); Project targets default to all open project issues. |
