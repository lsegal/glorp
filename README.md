...
Naming no model runs the agent's managed default -- `gpt-5.6-terra` for `codex`, `sonnet` for `claude`, `gemini-3.5-flash` for `gemini`, `claude-sonnet-4-6` for `agy` -- rather than whatever its CLI reaches for on its own, which is usually its largest model. `glorp agents` prints each default, and a definition can set or drop its own with `"defaultModel"` in `--config`.
...
Handled issues and active sessions are stored in `.glorp.json` by default. This prevents duplicate work after a restart and allows glorp to resume an interrupted session with the original agent, even when the new process selects a different `--agent`. If the original working directory is gone, the resumed agent is told to regenerate its missing work. Two cases restart the work instead of resuming it. Work pinned to an agent the current configuration no longer dispatches to is not resumed with the retired agent: the persisted session is discarded, logged as `discarded persisted agent "NAME"; it is no longer configured`, and the issue is redispatched to a currently configured agent with a fresh session. Agents whose definition declares `"session": {"assign": "none"}` -- `opencode`, `cline`, and `agy` among the built-ins -- have no resumable session at all, so picking their work back up always restarts the agent with the recovery prompt; because `gh-fix` is re-entrant, the restarted run adopts the branch and draft pull request the previous one left open rather than starting the issue over. Issues that declare dependencies using `depends on #123` or GitHub's issue-dependency relationship remain blocked until those dependencies close, with one exception: when the repository has [GitHub stacked pull requests](https://github.com/github/gh-stack) enabled and an issue is blocked by exactly one open issue that already has an open pull request, glorp dispatches it anyway and `gh-fix` stacks its pull request on the blocker's branch, rebasing it with `gh stack` as the blocker changes and merging it only after the blocker merges. A stacked run whose pull request is ready and green while the blocker is still open is parked instead of holding a concurrency slot: glorp records it as parked rather than completed, leaves it alone while the blocker stays open, and dispatches it again once the blocker closes so `gh-fix` can retarget and merge it.
...
| --- | --- | --- |
| `--agent AGENT[/MODEL][:LEVEL]` | `codex` | Agent to run, with an optional model and reasoning level, such as `claude`, `claude/opus`, or `codex/gpt-5.6:high`. glorp ships definitions for `codex`, `claude`, `gemini`, `muse`, `opencode`, `cline`, and `agy`; `glorp agents` lists the ones in force. Each accepts the levels its own definition names -- `low`, `medium`, and `high` for `codex`, `claude`, `opencode`, and `agy`, those plus `none` and `xhigh` for `cline`, and `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `ultra` for `muse` -- and a level outside that set is rejected here with the list. Any agent declared in `--config` is accepted too, on the models and levels its own definition lists; see the [agent definitions reference](https://lsegal.github.io/glorp/agents/) for how to add one. An agent whose CLI has no reasoning-effort flag -- `gemini` is the built-in one -- declares an empty level list and rejects a level outright, so `--agent gemini:high` stops the run naming the agent instead of accepting a level it has nowhere to pass. An agent given no model runs its definition's `defaultModel` -- `gpt-5.6-terra` for `codex`, `sonnet` for `claude`, `gemini-3.5-flash` for `gemini`, and `claude-sonnet-4-6` for `agy`, so a queue of issues does not silently land on the CLI's own largest model. `agy` is the one whose default is picked around its own argv: it renders a level into `--effort` and almost every id in its catalog spells the level into the id itself (`gemini-3.8-flash-medium`), so the default is the mid-tier id that carries none rather than one that would contradict the `--effort` glorp passes beside it. The three built-ins whose catalogs are fetched for the signed-in account (`muse`, `cline`, `opencode`) name no default and leave the choice to the CLI, because an id glorp wrote down would fail the dispatch outright on an account that does not carry it; `--config` can set or drop one per agent. Repeatable; when given more than once, new issues are load balanced evenly across the listed agents, each using its own model and level. |
