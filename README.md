...
| `--web-ui-public` | `false` | Publish the browser dashboard through an ngrok tunnel so a run can be watched from a phone or another machine. **What is exposed, and to whom:** the published dashboard shows everything the local one does — the watched repositories, the issue numbers and titles in flight, each clone path and session ID, and every line the agents print, including anything an agent happens to print that you would not put in a public place — to anyone holding the link. That link is the credential: glorp generates a random token per run, prints the URL carrying it once to the terminal at startup, and refuses every request that arrives without it. Opening the link stores the token in a cookie and drops it from the address bar; scripts can send it as `Authorization: Bearer` instead. The published view is **read-only**: the retry, stop, and settings controls are absent from it, and the endpoints behind them refuse writes, so the link cannot be used to steer the run. It needs `--ui web`, and it does not change the local dashboard, which stays bound to localhost, unauthenticated, and reachable through `glorp ui` exactly as it is without the flag. A webhook run publishes over the tunnel it already starts, on the same public host as `--webhook-path`; a `--poll` or `--browser` run, which has no tunnel, starts one for the dashboard alone. ngrok's free plan shows its browser interstitial on the first visit, so expect one extra tap before the dashboard appears. |
