...
// browserCommentsTab is the name of the one tab every comment read is made in.
...
const browserCommentsTab = "comments"

// browserCommentSource reads an issue's or pull request's comments off its
...
type browserCommentSource struct {
	// pageFor opens (or returns the already-open) tab comments are read in.
	pageFor func(name string) (browserPage, error)
...
// browserCommentRow is one comment as the page script reports it.
type browserCommentRow struct {
	ID        string `json:"id"`
...
// browserCommentList is the page script's result: the comments it found and
// whether it recognised the page as a conversation at all.
type browserCommentList struct {
	Comments   []browserCommentRow `json:"comments"`
	Recognized bool                `json:"recognized"`
}

// newBrowserComments builds the comment client browser mode runs the handoff
...
func newBrowserComments(browser *Browser, api CommentClient, logf func(string, ...interface{})) *browserCommentSource {
...
func (s *browserCommentSource) PostComment(ctx context.Context, repo string, number int, body string) error {
	return s.api.PostComment(ctx, repo, number, body)
}

// browserCommentsURL is the conversation page for an issue or a pull request.
...
func browserCommentsURL(repo string, number int) string {
...
func (s *browserCommentSource) ListComments(ctx context.Context, repo string, number int) ([]Comment, error) {
	pageURL := browserCommentsURL(repo, number)
...
func (s *browserCommentSource) readComments(ctx context.Context, pageURL string) ([]Comment, error) {
	s.tab.Lock()
	defer s.tab.Unlock()
	page, err := s.pageFor(browserCommentsTab)
	if err != nil {
		return nil, err
	}
	...
	for attempt := 0; ; attempt++ {
		var read browserCommentList
		if err := page.Eval(browserCommentsScript, &read); err != nil {
	...
	if len(list.Comments) == 0 && browserSignedOut(page) {
		return nil, fmt.Errorf("read conversation at %s: the browser profile is signed out", pageURL)
	}
	return commentsFromBrowserRows(pageURL, list.Comments)
}

// commentsFromBrowserRows converts the extracted rows into Comments, in
...
func commentsFromBrowserRows(pageURL string, rows []browserCommentRow) ([]Comment, error) {
	...
	for _, row := range rows {
		...
		comments = append(comments, Comment{Body: row.Body, Author: row.Author, CreatedAt: createdAt})
	}
...
func (s *browserCommentSource) visit(page browserPage, pageURL string) error {
	s.mu.Lock()
...
func (s *browserCommentSource) attempts() int {
...
func (s *browserCommentSource) delay() time.Duration {
	if s.settleDelay > 0 {
...
func (s *browserCommentSource) pause(ctx context.Context) bool {
	if s.sleep != nil {
...
func (s *browserCommentSource) reportFallback(pageURL string, cause error) {
	s.mu.Lock()
