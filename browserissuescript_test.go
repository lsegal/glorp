...
// browserFixtureTab serves the captured issues-page fixtures over loopback and
...
func browserFixtureTab(t *testing.T) (*BrowserTab, string) {
	t.Helper()
	...
	t.Cleanup(cancel)
	browser, err := startBrowser(ctx, browserConfig{Profile: t.TempDir()})
	if err != nil {
...
// of it.
func extractFixture(t *testing.T, tab *BrowserTab, baseURL, fixture string) browserIssueList {
	t.Helper()
	if err := tab.Navigate(baseURL + "/" + fixture); err != nil {
		t.Fatalf("navigate to %s: %v", fixture, err)
	}
	var list browserIssueList
	if err := tab.Eval(browserIssueRowsScript, &list); err != nil {
...
func TestExtractorScriptAgainstFixtures(t *testing.T) {
	...
	t.Run("signed-in list", func(t *testing.T) {
		...
		if got := nextBrowserIssuesURL(list.Next); got != list.Next {
			t.Fatalf("nextBrowserIssuesURL rejected the pager's own target %q", list.Next)
		}
	})
