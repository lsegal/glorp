...
10. Never merge a stacked pull request while its base is anything other than the default branch. Once it is marked ready and CI passes and the blocker's pull request is still open, end the run without waiting for the blocker to merge: post one comment on the pull request saying it is ready and parked on the blocker's pull request, then report the parked pull request as the outcome rather than as a failure or a blocker. Do not create follow-up issues or close the issue. When glorp dispatched the run, glorp parks it, frees its agent slot, and dispatches the issue again once the blocker is no longer open. A later run resumes this pull request, follows step 9, drives CI, and continues with "Merge and verify". If the blocker already merged by the time CI passes, follow step 9 in this run instead of ending it.
