...
func newRecoveryHarness(t *testing.T, headed error, login func() (string, error)) *recoveryHarness {
	...
	h.recovery = &browserSignInRecovery{
		config:  browserConfig{Profile: "/tmp/profile"},
		...
		login: func(_ context.Context, config browserConfig, _ writerFunc, _ time.Duration) (string, error) {
			h.logins++
...
func TestBrowserSignedOutStatusClassification(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "404 on a signed-out page is a sign-in signal", status: 404, signIn: browserSignInState{SignedOut: true}, want: true},
		{name: "403 on a signed-out page is a sign-in signal", status: 403, signIn: browserSignInState{SignedOut: true}, want: true},
		{name: "404 on a signed-in page is a real 404", status: 404, signIn: browserSignInState{SignedIn: true, Login: "lsegal"}, want: false},
		{name: "404 with no evidence either way blames nothing", status: 404, want: false},
		{name: "500 is never a sign-in signal", status: 500, signIn: browserSignInState{SignedOut: true}, want: false},
		{name: "200 is never a sign-in signal", status: 200, signIn: browserSignInState{SignedOut: true}, want: false},
	} {
		t.Run(test.name, func(t *testing.T) {
			page := &fakeBrowserPage{status: test.status, signIn: test.signIn}
			if got := browserSignedOutStatus(page, test.status); got != test.want {
				t.Fatalf("browserSignedOutStatus(%d) = %v, want %v", test.status, got, test.want)
			}
...
func TestBrowserIssueSourceReportsSignedOutOn404(t *testing.T) {
	page := &fakeBrowserPage{status: 404, signIn: browserSignInState{SignedOut: true}}
...
func TestBrowserBoardReportsSignedOutOn404(t *testing.T) {
	page := &fakeBrowserPage{status: 404, signIn: browserSignInState{SignedOut: true}}
	board := &BrowserBoard{Page: func(string) (browserPage, error) { return page, nil }, Profile: "/tmp/profile"}
...
func TestBrowserSignInRecoveryOpensWindowAndResumes(t *testing.T) {
	h := newRecoveryHarness(t, nil, func() (string, error) { return "lsegal", nil })
	h.recovery.attempt(context.Background(), &browserSignedOutError{URL: "https://github.com/lsegal/private/issues", Profile: "/tmp/profile", Status: 404})
...
func TestBrowserSignInRecoveryBacksOffAfterFailedLogin(t *testing.T) {
	h := newRecoveryHarness(t, nil, func() (string, error) { return "", errors.New("timed out") })
	signedOut := &browserSignedOutError{URL: "https://github.com/lsegal/private/issues", Profile: "/tmp/profile"}
	...
	h.now = h.now.Add(browserSignInBackoff)
	h.recovery.attempt(context.Background(), signedOut)
	if h.logins != 2 {
		t.Fatalf("did not retry after the back-off elapsed: %d login(s)", h.logins)
	}
	if want := h.now.Add(2 * browserSignInBackoff); !h.recovery.next.Equal(want) {
		t.Fatalf("next attempt at %v, want the doubled back-off at %v", h.recovery.next, want)
	}
}

func TestBrowserSignInRecoveryRefusesWithoutADisplay(t *testing.T) {
...
func TestBrowserSignInRecoveryIgnoresOtherFailures(t *testing.T) {
	h := newRecoveryHarness(t, nil, func() (string, error) { return "lsegal", nil })
...
func (s *guardedIssueSource) ListIssues(context.Context, string) ([]Issue, error) {
	s.calls++
	return nil, s.err
}

func TestBrowserSignInGuardRecoversAndStillFailsThePoll(t *testing.T) {
	h := newRecoveryHarness(t, nil, func() (string, error) { return "lsegal", nil })
	source := &guardedIssueSource{err: &browserSignedOutError{URL: "https://github.com/lsegal/private/issues"}}
	guard := browserSignInGuard{source: source, recovery: h.recovery}
	if _, err := guard.ListIssues(context.Background(), "lsegal/private"); !errors.Is(err, errBrowserSignedOut) {
