...
func TestApplyBrowserSourcesInjectsBrowserReaders(t *testing.T) {
	...
	applyBrowserSources(w, &Browser{}, "is:issue state:open", true)
	...
	if repos.filter != "is:issue state:open" || !repos.allIssues {
	...
	if issues.Board.Filter != "is:issue state:open" || !issues.Board.AllIssues {
...
func TestApplyBrowserSourcesWithoutBrowserKeepsAPISources(t *testing.T) {
	gh := GHCLI{Binary: "gh"}
	w := &Glorp{Issues: gh, Discussions: gh, Status: gh, Comments: gh, Projects: gh, Out: io.Discard}
	applyBrowserSources(w, nil, "is:issue state:open", true)
