...
func pollTicks(t *testing.T, source *browserIssueSource, ticks int) error {
	t.Helper()
	var lastErr error
	for i := 0; i < ticks; i++ {
...
// steady state.
func TestBrowserVisionNeverCalledWhileExtractionWorks(t *testing.T) {
	...
	page := &visionPage{result: browserIssueList{
		Recognized: true,
		Rows:       []browserIssueRow{{Number: 12, Title: "one"}},
	}}
	source := newTestIssueSource(&fakeBrowserPage{}, "", false, nil)
...
// either.
func TestBrowserVisionNeverCalledForAnEmptyList(t *testing.T) {
	clock := &visionClock{step: browserWatchInterval}
	vision, asks, _ := testVision(t, browserVisionRunLimit, browserVisionCooldown, clock, []int{7})
	page := &visionPage{result: browserIssueList{Recognized: true, Empty: true}}
	source := newTestIssueSource(&fakeBrowserPage{}, "", false, nil)
	source.pageFor = func(string) (browserPage, error) { return page, nil }
	source.vision = vision
...
func TestBrowserVisionNeverCalledForANavigationFailure(t *testing.T) {
	clock := &visionClock{step: browserWatchInterval}
	vision, asks, _ := testVision(t, browserVisionRunLimit, browserVisionCooldown, clock, []int{7})
	page := &visionPage{status: 404, result: browserIssueList{Recognized: true}}
	source := newTestIssueSource(&fakeBrowserPage{}, "", false, nil)
	source.pageFor = func(string) (browserPage, error) { return page, nil }
	source.vision = vision
...
func TestBrowserVisionCooldownHoldsUnderRepeatedFailures(t *testing.T) {
	...
	// cooldown alone.
	vision, asks, _ := testVision(t, 1000, browserVisionCooldown, clock, []int{7})
	page := &visionPage{result: browserIssueList{}}
	source := newTestIssueSource(&fakeBrowserPage{}, "", false, nil)
	source.pageFor = func(string) (browserPage, error) { return page, nil }
	source.vision = vision
...
func TestBrowserVisionRunCapHoldsUnderRepeatedFailures(t *testing.T) {
	// A step far longer than the cooldown means only the cap can stop this.
	clock := &visionClock{step: time.Hour}
	vision, asks, logs := testVision(t, browserVisionRunLimit, browserVisionCooldown, clock, []int{7})
	page := &visionPage{result: browserIssueList{}}
	source := newTestIssueSource(&fakeBrowserPage{}, "", false, nil)
	source.pageFor = func(string) (browserPage, error) { return page, nil }
	source.vision = vision
	...
	if *asks != browserVisionRunLimit {
		t.Fatalf("expected the run capped at %d vision calls, got %d", browserVisionRunLimit, *asks)
	}
	if page.screenshots != browserVisionRunLimit {
		t.Fatalf("expected %d screenshots, got %d", browserVisionRunLimit, page.screenshots)
	}
...
func TestBrowserVisionCooldownIsPerTarget(t *testing.T) {
	clock := &visionClock{step: browserWatchInterval}
	vision, asks, _ := testVision(t, 1000, browserVisionCooldown, clock, []int{7})
	source := newTestIssueSource(&fakeBrowserPage{}, "", false, nil)
	source.pageFor = func(string) (browserPage, error) { return &visionPage{}, nil }
	source.vision = vision
...
func TestBrowserVisionRecoveredNumbersBecomeIssues(t *testing.T) {
	clock := &visionClock{step: browserWatchInterval}
	vision, asks, _ := testVision(t, browserVisionRunLimit, browserVisionCooldown, clock, []int{412, 398})
	source := newTestIssueSource(&fakeBrowserPage{}, "", false, nil)
	source.pageFor = func(string) (browserPage, error) { return &visionPage{}, nil }
	source.vision = vision
...
func TestBrowserVisionDiscardsAnUnparseableAnswerWithoutRetrying(t *testing.T) {
	clock := &visionClock{step: browserWatchInterval}
	asks := 0
	vision := &browserVision{
...
func TestBrowserVisionChargesForAFailedScreenshot(t *testing.T) {
...
func TestBrowserIssueSourceWithoutVisionNeverScreenshots(t *testing.T) {
	page := &visionPage{}
	source := newTestIssueSource(&fakeBrowserPage{}, "", false, nil)
	source.pageFor = func(string) (browserPage, error) { return page, nil }

	if _, err := source.ListIssues(context.Background(), "owner/repo"); !errors.Is(err, errBrowserExtraction) {
...
func TestParseBrowserVisionNumbers(t *testing.T) {
	cases := []struct {
		name    string
		output  string
		want    []int
...
func TestBrowserVisionCapHoldsAcrossConcurrentTargets(t *testing.T) {
	clock := &visionClock{step: time.Hour}
	vision, asks, _ := testVision(t, browserVisionRunLimit, browserVisionCooldown, clock, []int{7})
	var mu sync.Mutex
	inner := vision.ask
