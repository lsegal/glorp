...
| `doctor.knownModels` | array of string | no | none | Model names the definition itself carries, for a CLI with no way at all to be asked. Claude is the only built-in that declares any: its current aliases and known full model ids are reported without validating `--agent`, since the CLI may accept other models too. Reported as `agent/model` names labelled *known to glorp; the CLI may accept others*. |
