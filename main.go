...
func watchFlagSet(agentSpecs *agentFlag, agentBinaries *agentBinaryFlag, filter *filterFlag) *flag.FlagSet {
	...
	flags.String("pollmode", "browser", "delivery mode: browser, webhook, or poll")
	flags.Bool("poll", false, "deprecated alias for -pollmode=poll")
	...
	flags.Bool("browser", false, "deprecated alias for -pollmode=browser")
...
func (g GHCLI) OriginatingWorkState(ctx context.Context, repo string, number int) (OriginatingWorkState, error) {
	...
	for _, event := range events {
		...
		base := currentPullRequest.Base
		stacked := base.Ref != "" && base.Repo.DefaultBranch != "" && base.Ref != base.Repo.DefaultBranch
		state.PullRequests = append(state.PullRequests, PullRequestWorkState{Number: pullRequest.Number, State: currentPullRequest.State, Merged: currentPullRequest.MergedAt != nil, IsDraft: currentPullRequest.Draft, Stacked: stacked})
	}
	return state, nil
}
